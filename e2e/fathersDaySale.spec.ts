...
// REAL clock (it never sees `?_now`) — during the actual sale window it says
// "FATHER'S DAY" on every page and would leak into unscoped text queries.
...
test.describe("Father's Day 2026 sale", () => {
  ...
  test('savings amount on $8 4-pack DOUBLES during sale (chip count + amount)', async ({ page }) => {
    ...
    await visitBuyTokens(page, SALE_MID)
    const saleChips = await page.locator('main').locator('text=/4-Pack bundle|Father.s Day/').count()
